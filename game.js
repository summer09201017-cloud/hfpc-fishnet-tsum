...
var MODES = {
  ...
  teen: { label:'青少年(12+)', types:9, minChain:5, target:153, r:32 }
};
var modeKey = 'kid';
...
var BURST = { icon:'🕸', name:'撒大網', banner:'🕸 撒下大網!一網滿滿收進來!' };
...
var TROUBLE = { id:'tr', name:'搗蛋河豚', c1:'#7a6a92', c2:'#54476a', fc:'#a898c0', ec:'#f4f1e8', trouble:true };
...
function maxLevel(){ try{ return Math.max(1, parseInt(localStorage.getItem('fishnet-lvl-'+modeKey))||1); }catch(e){ return 1; } }
function loadStars(){ try{ stars = JSON.parse(localStorage.getItem('fishnet-stars-'+modeKey)||'{}')||{}; }catch(e){ stars = {}; } }
function saveStars(lv, st){
  if ((stars[lv]|0) >= st) return;
  stars[lv] = st;
  try{ localStorage.setItem('fishnet-stars-'+modeKey, JSON.stringify(stars)); }catch(e){}
}
...
function sprintEnd(){
  ...
  speak('win');
  if (!doneSent){ doneSent = true; if (window.__ping) window.__ping('fishnet-tsum-done', Math.round((Date.now()-startTime)/1000)); }
}
...
function bgmSeed(lv){
  ...
  var x = 2166136261, s = 'fishnet-tsum|' + (lv || 1);
...
function bgmTick(){
  ...
  var i = bgmStep % 16;
  blip(BASS[i], 0.26, 'sine', 0.02);
  if (bgmStep % 2 === 0) { var mf = MELO[(bgmStep/2)%16|0]; if (mf) blip(mf, 0.22, MELW, MELVOL); }
  bgmStep++;
}
...
function spawnTsum(){
  ...
  if (Math.random() < 0.05){ t = BIGFISH; r = M.r*1.3; }   // 大魚:稀有特件
  else {
    // 07-22:群聚生成——45% 抄場上隨機一顆的型別、落在它附近,讓 5+ 長鏈自然可達
    anchor = playing && tsums.length && Math.random() < (modeKey==='teen'?0.25:modeKey==='kid'?0.35:0.45) ? tsums[(Math.random()*tsums.length)|0] : null;   // 07-22:開場鋪場不群聚(滾雪球整片同色)
...
function chainType(){
  for (var i=0;i<chain.length;i++) if (!chain[i].t.wild && !chain[i].t.trouble) return chain[i].t;
  return null;
}
function canLink(t){
  var ct = chainType();
  return t.t.wild || t.t.trouble || !ct || t.t === ct;
}
function evPos(e){
...
function collect(list){
  ...
  fed = Math.max(0, (sprint ? fed + caught : Math.min(curTarget, fed + caught)) + goldN*mult - troubles*TROUBLE_PENALTY);
  chainCount++;
  hintT = 0; hintGroup = null;
  chordCollect(n);
  for (i=0;i<n;i++){
  ...
  else if (goldN > 0){ banner = { text:'✨ 金色雙倍!多收 ' + (goldN*mult) + ' 條魚', t:1.5 }; }
...
function drawDisciple(x, y, s, i, t){
  var happyN = Math.floor((fed/curTarget)*BOAT_N);
  var happy = i < happyN;
...
function drawHUD(){
  ...
  if (sprint){ ctx.fillText('⏱ ' + Math.ceil(sprintT) + ' 秒・收 ' + Math.round(shownFed) + ' 條魚', W/2, 40); }
  else { ctx.fillText('已收 ' + Math.round(shownFed) + ' / ' + curTarget + ' 條魚', W/2, 40); }
...
function hudTap(p){
  if (p.y < CROWD_TOP){
    ...
    if (p.x > W-56){ muted = !muted; try{ localStorage.setItem('fishnet-mute', muted?'1':'0'); }catch(e){} return true; }
  }
  return false;
}
...
function drawScene(t){
  ...
  // 船上收成的魚堆(進度看得見)
  var pile = Math.floor((fed/curTarget)*10);
  for (i=0;i<pile;i++){
...
function drawMenu(t){
  ...
  for (i=0;i<3;i++){
    ...
    ctx.fillText('同款連 ' + MODES[keys[i]].minChain + ' 顆・收滿 ' + MODES[keys[i]].target + ' 條', W/2, y+60);
...
function drawWin(t){
  ...
  ctx.fillText(sprint ? ('⏱ 時間到!共收 ' + fed + ' 條魚') : '🎉 網拉到岸上了!', W/2, 170);
...
function winTap(p){
  for (var i=0;i<winBtns.length;i++){
    var b = winBtns[i];
    if (p.x>b.x && p.x<b.x+b.w && p.y>b.y && p.y<b.y+b.h){
      if (b.act==='next'){ var nx = level+1; try{ localStorage.setItem('fishnet-lvl-'+modeKey, ''+Math.max(maxLevel(), nx)); }catch(e){} startGame(nx); return; }
